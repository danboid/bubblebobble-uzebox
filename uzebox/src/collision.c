...
uint8_t can_stand_at(int16_t x, int16_t y, uint8_t width) {
    ...
    if (is_tile_semi_solid(&game.level, grid_x, grid_y)) {
        return 1;
    }
...
int16_t get_ground_height(int16_t x, uint8_t width) {
    ...
    for (int8_t y = grid_y; y < GRID_SIZE_Y; y++) {
        if (is_tile_solid(&game.level, grid_x, y)) {
            return y * TILE_HEIGHT;
        }
        if (is_tile_semi_solid(&game.level, grid_x, y)) {
            return y * TILE_HEIGHT;
        }
    }
...
void check_bubble_tile_collisions(void) {
    for (uint8_t b = 0; b < 8; b++) {
        ...
        if (bubble->base.vx > 0 && is_tile_solid(&game.level, grid_x + 1, grid_y)) {
...
void update_dead_enemies(void) {
    for (uint8_t i = 0; i < game.dead_enemy_count; i++) {
        ...
        if (is_tile_solid(&game.level, grid_x, grid_y) || grid_y >= GRID_SIZE_Y) {
